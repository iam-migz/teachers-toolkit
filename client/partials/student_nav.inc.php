...
<nav class="navbar navbar-expand-lg navbar-dark default-color sticky-top">
  <a class="navbar-brand" rel="next" href="<?=PATH?>/student/home.php">Teachers Toolkit</a>
...
<script>
  ...
  function showProfile(account) {
    modalBody.innerHTML = `
    <h2 style="text-align: center; margin: 1em 0;">Student Account</h2>
    <div style="display: flex; justify-content: center;">
      ...
      <div style="width: 35%; ">
        ...
        <p>${account.gender}</p>
        <p>${account.email}</p>
        <p>${account.barangay}, ${account.city}, ${account.province}</p>
        <p>${account.LRN}</p>
  ...
  axios.get('http://localhost/teachers-toolkit-app/server/student/findOne/<?=$_SESSION['account_id']?>')
    .then(data => {
      data = data.data;
      if (data.result === 1) {
